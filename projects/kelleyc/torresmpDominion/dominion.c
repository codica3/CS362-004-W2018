...
int adventurerCard(struct gameState *state, int* temphand) {
  ...
  while( drawntreasure < 2 ){
    ...
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold) { /* **Fixed this line so code would compile** */
      drawntreasure++;
    }
    else {
...
int feastCard(struct gameState *state, int* temphand, int choice1) {
  int i, x;
  int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      return adventurerCard(state, temphand);
    ...
    case feast:
      return feastCard(state, temphand, choice1);
