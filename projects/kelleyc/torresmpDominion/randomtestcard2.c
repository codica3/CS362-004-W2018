...
**
** Original inputs: int current player (cp), struct valid gameState *s, and int
** handPos (hp).
**
...
**
** New inputs: int card, int choice1, int choice2, int choice3, struct valid
** gameState *state, int handPos, and int* bonus.
********************************************************************************/
...
int checkGreat_hall(int p, struct gameState *G) {
	...
	//check for card shuffle
	if (G->deckCount[currentPlayer] < 1) {

		shuffled = G->discardCount[currentPlayer];
	}
